...
execute if data storage slimecore:_ v.uninstall.args{force:true} run return run function slimecore:_/impl/-/uninstall_slimecore/confirm
...
data modify storage slimecore:_ v.uninstall.msg append value {text:"\n", color:dark_purple, bold:false, extra:[{text:"It is highly advised to ensure no datapack-path related rebuild errors exist before uninstalling SlimeCore; not doing so may result in unexpected and/or undesirable behavior."}]}
