...
data modify storage slimecore:_ var.build.this_order_entry set from storage slimecore:_ var.build.current_order[-1]
scoreboard players remove *build.insertion_index _slimecore 1

function slimecore:_/build/pass_order/loads/order/fulfill_requirement with storage var.build
